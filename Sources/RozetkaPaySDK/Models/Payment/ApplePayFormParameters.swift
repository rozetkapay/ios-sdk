...
/// Represents the parameters required to initiate an Apple Pay payment
/// via the embeddable `ApplePayFormView`.
/// Conforms to `ParametersProtocol`.
///
/// Unlike `PaymentParameters`, this configuration is Apple Pay only:
/// no card form is rendered, so `applePayConfig` is required.
public struct ApplePayFormParameters: ParametersProtocol {
    ...
    /// Creates a new instance of `ApplePayFormParameters`.
    public init(
